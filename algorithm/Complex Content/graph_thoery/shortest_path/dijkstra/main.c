    ...
    for(i=1;i<=g_total_node;i++){
     ...
     while(heap->heap_size > 0){
         ...
         for(i=1;i<=g_total_node;i++) {
             ...
             if(adj_pos &&g_graph[node_num][i] ){/*g_graph[node_num][i] is essential*/
                 heap->array[adj_pos].parent = node_num;
                 if(heap->array[adj_pos].key > min.key + g_graph[node_num][i]){
                     heap_decrease_key(heap,adj_pos,g_graph[node_num][i]+min.key);
                 }
             }
         }
     }
